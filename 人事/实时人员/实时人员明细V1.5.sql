...
FROM
    ...
    SELECT
		...
		and hrb.pk_psncl in ('01','02','03','04')
        and if(dismiss.psncode is not null and hrj.entry_date<dismiss.effect_date,
            date(FROM_UNIXTIME(dismiss.effect_date))>=@time,if(dismiss.psncode is not null and hrj.entry_date>dismiss.effect_date,
                @time not between date(FROM_UNIXTIME(dismiss.effect_date))and date(FROM_UNIXTIME(hrj.entry_date)),1=1))
