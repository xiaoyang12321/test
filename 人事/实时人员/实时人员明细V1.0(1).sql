 ...
 SELECT 
    ...
    CASE
        ...
        WHEN a.educational_background = 08 THEN '初级及以下'
        ELSE ''
    END AS education,
    CASE
    ...
    IFNULL(c.current_wage, 0) AS current_wage,
    d.labor_mode,
    e.insured_place
FROM
    (
    ...
    SELECT 
        hrb.bind_sid,
            ...
            hrb.id_number,
            hrb.sex,
            FROM_UNIXTIME(hrb.birthday, '%Y-%m-%d') AS birthday,
    ...
    WHERE
        FIND_IN_SET(hrb.bind_sid, @SID) > 0
            AND IF(dismiss.psncode IS NOT NULL, DATE(FROM_UNIXTIME(dismiss.effect_date)) >= @END_DAY, 1 = 1)
    GROUP BY name
        ...
        LEFT JOIN
    (SELECT 
        j.name, j.psncode, d.name AS type_name
    FROM
        erp.hr_jobinfo j
    JOIN erp.taxonomy_term_data d ON j.position_name = d.tid) AS b ON a.psncode = b.psncode
        LEFT JOIN
    (SELECT 
        w.name,
            w.psncode,
    ...
    FROM
        erp.hr_wageinfo w
    LEFT JOIN erp.taxonomy_term_data d ON w.payroll_bank = d.tid) AS c ON a.psncode = c.psncode
        LEFT JOIN
    (SELECT 
        j.name, j.psncode, d.name AS labor_mode
    FROM
        erp.hr_jobinfo j
    JOIN erp.taxonomy_term_data d ON j.job_type = d.tid) AS d ON a.psncode = d.psncode
        LEFT JOIN
    (SELECT 
        b.name, b.psncode, d.name AS insured_place
    FROM
        erp.hr_basicinfo b
    JOIN erp.taxonomy_term_data d ON b.social_security_address = d.tid) AS e ON a.psncode = e.psncode
        LEFT JOIN
    (SELECT 
    ...
    WHERE
        IF(LENGTH(@sid) > 0, FIND_IN_SET(rcs.sid, @sid) > 0, '1=1')) store_info
    LEFT JOIN (SELECT 
    ...
    WHERE
        rcst1.category_name = 'market') AS market ON store_info.dudao_id = market.tid) AS store_base_info ON store_base_info.sid = a.bind_sid
WHERE
    entry_date <= @END_DAY
